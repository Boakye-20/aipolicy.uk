...
export default function PolicyExplorer() {
    ...
    useEffect(() => {
        applyFilters();
    }, [policies, searchTerm, filters, sortOrder]);
    ...
    // Freshness indicator: the most recent publication date across all policies.
    const lastUpdated = (() => {
        const times = policies
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex-1 flex flex-wrap items-center justify-between gap-2">
                    <p className="text-blue-800">
                        <span>Showing <strong>{filteredPolicies.length}</strong> {filteredPolicies.length === 1 ? 'policy' : 'policies'}</span>
                        {searchTerm && <span> matching "{searchTerm}"</span>}
                    </p>
                    {lastUpdated && (
                        <span className="text-xs text-blue-700">Data current as of {lastUpdated}</span>
                    )}
                </div>
            </div>
            ...
            <div className="space-y-4">
                {currentPolicies.length === 0 ? (
                    <div className="bg-white rounded-lg shadow border border-gray-200 p-12 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-gray-900 mb-1">No policies match your filters</h3>
                        <p className="text-gray-600 mb-4">Try removing a filter or broadening your search.</p>
                        <button
                            onClick={clearFilters}
