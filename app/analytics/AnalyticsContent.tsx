...
export default function AnalyticsContent({ initialPolicies }: AnalyticsContentProps) {
    ...
    const getTimelineData = () => {
        const grouped = policies.reduce((acc, policy) => {
            ...
            if (!acc[formattedDate]) {
                acc[formattedDate] = { period: formattedDate, date, total: 0, regulations: 0, research: 0, strategic: 0, guidance: 0 };
            }
            acc[formattedDate].total++;
            if (policy.policy_type === 'Regulation & Compliance') acc[formattedDate].regulations++;
            else if (policy.policy_type === 'Research & Analysis') acc[formattedDate].research++;
            else if (policy.policy_type === 'Strategy & Frameworks') acc[formattedDate].strategic++;
            else if (policy.policy_type === 'Implementation Guidance') acc[formattedDate].guidance++;
            return acc;
    ...
    const getPolicyTypeBreakdown = () => {
        ...
        return Object.entries(typeData)
            .map(([name, value]) => ({ name: nameMap[name] || name, value, originalName: name, color: colorMap[name] || COLORS.gray }))
    ...
    const getDepartmentActivity = () => {
        ...
        const topDepts = Object.entries(deptActivity).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([dept]) => dept);
        ...
        return last12Months.map(month => {
            ...
            topDepts.forEach(dept => {
                periodData[dept] = policies.filter(p => p.year_month === month && p.dept === dept).length;
            });
            return periodData;
        });
    };
    ...
    const getSectorTrends = () => {
        ...
        if (otherCount > 0) result.push({ sector: 'Other', count: otherCount, fullName: 'Other sectors' });
        return result;
    };
    ...
    const calculateTrend = () => {
        ...
        if (previous === 0) return { direction: 'up', percentage: 100, recentCount: recent, previousCount: 0, period: '3 months' };
        ...
        <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between">
                        ...
                        <div className={`p-3 rounded-lg ${trend.direction === 'up' ? 'bg-green-50 text-green-600' : trend.direction === 'down' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}>
                        ...
                        <div>
                            ...
                            <div className="mt-2 flex -space-x-1">
                                {policyTypeBreakdown.slice(0, 5).map((item, index) => (
                                    <div key={index} className="h-2 rounded-full" style={{ width: `${(item.value / Math.max(...policyTypeBreakdown.map(p => p.value))) * 100}%`, backgroundColor: item.color, marginRight: '2px' }} />
                                ))}
                            </div>
                        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 mb-8 border border-slate-200">
                ...
                <div className="h-[400px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={timelineData} margin={{ top: 10, right: 30, left: 0, bottom: 30 }}>
                            <defs>
                                {[['colorRegulations', '#ef4444'], ['colorStrategic', '#3b82f6'], ['colorResearch', '#10b981'], ['colorGuidance', '#06b6d4']].map(([id, color]) => (
                            ...
                            <XAxis dataKey="period" tick={{ fill: '#6b7280', fontSize: 12 }} tickLine={false} axisLine={{ stroke: '#e5e7eb' }} tickMargin={10} interval="preserveStartEnd" />
                            <YAxis tick={{ fill: '#6b7280', fontSize: 12 }} tickLine={false} axisLine={false} tickFormatter={formatYAxis} width={30} />
                            <Tooltip content={renderCustomTooltip} cursor={{ stroke: '#d1d5db', strokeWidth: 1, strokeDasharray: '3 3' }} />
                            <Area type="monotone" dataKey="regulations" name="Regulations" stackId="1" stroke="#ef4444" fillOpacity={1} fill="url(#colorRegulations)" strokeWidth={2} activeDot={{ r: 6, strokeWidth: 2 }} />
                            <Area type="monotone" dataKey="strategic" name="Strategic" stackId="1" stroke="#3b82f6" fillOpacity={1} fill="url(#colorStrategic)" strokeWidth={2} activeDot={{ r: 6, strokeWidth: 2 }} />
                            <Area type="monotone" dataKey="research" name="Research" stackId="1" stroke="#10b981" fillOpacity={1} fill="url(#colorResearch)" strokeWidth={2} activeDot={{ r: 6, strokeWidth: 2 }} />
                            <Area type="monotone" dataKey="guidance" name="Guidance" stackId="1" stroke="#06b6d4" fillOpacity={1} fill="url(#colorGuidance)" strokeWidth={2} activeDot={{ r: 6, strokeWidth: 2 }} />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
                    ...
                    <div className="h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={policyTypeBreakdown} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" nameKey="name" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                                    {policyTypeBreakdown.map((entry, index) => (
                                        <Cell key={`cell-${index}`} fill={entry.color} />
                                    ))}
                                </Pie>
                                <Tooltip content={renderCustomTooltip} formatter={(value: number, name: string, props: any) => [value, props.payload.originalName || name]} />
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
                    ...
                    <div className="h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={sectorTrends} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                ...
                                <YAxis dataKey="sector" type="category" width={120} tick={{ fill: '#6b7280', fontSize: 12 }} tickLine={false} axisLine={false} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
                    ...
                    <div className="h-[350px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={departmentActivity} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                                <XAxis dataKey="period" tick={{ fill: '#6b7280', fontSize: 12 }} tickLine={false} axisLine={{ stroke: '#e5e7eb' }} tickMargin={10} />
                                <YAxis tick={{ fill: '#6b7280', fontSize: 12 }} tickLine={false} axisLine={false} tickFormatter={formatYAxis} width={30} />
                                <Tooltip content={renderCustomTooltip} cursor={{ stroke: '#d1d5db', strokeWidth: 1, strokeDasharray: '3 3' }} />
                                <Legend layout="horizontal" verticalAlign="top" align="center" height={40} formatter={renderColorfulLegendText} />
                                {departmentActivity.length > 0 && Object.keys(departmentActivity[0]).filter(key => key !== 'period').map((dept, index) => (
                                    <Line key={dept} type="monotone" dataKey={dept} stroke={`hsl(${index * 60}, 70%, 50%)`} strokeWidth={2} dot={{ r: 2 }} activeDot={{ r: 5, strokeWidth: 0 }} name={dept} />
                                ))}
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
                    ...
                    <div className="h-[350px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={regulationsBySector} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                <CartesianGrid horizontal={true} vertical={false} strokeDasharray="3 3" stroke="#e5e7eb" />
                                <XAxis type="number" tick={{ fill: '#6b7280', fontSize: 12 }} tickLine={false} axisLine={false} />
                                <YAxis dataKey="sector" type="category" width={120} tick={{ fill: '#6b7280', fontSize: 12 }} tickLine={false} axisLine={false} />
                                <Tooltip content={({ active, payload }: any) => {
                                ...
                                <Bar dataKey="count" name="Regulations" radius={[0, 4, 4, 0]} fill="#ef4444">
                                    {regulationsBySector.map((_, index) => (
                                        <Cell key={`cell-${index}`} fill={`rgba(239, 68, 68, ${0.6 + (index * 0.08)})`} />
                                    ))}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                        {
                            color: trend.direction === 'up' ? 'bg-green-500' : trend.direction === 'down' ? 'bg-red-500' : 'bg-blue-500',
                            title: 'Publication Trend',
                            text: `Activity has ${trend.direction === 'up' ? 'increased' : trend.direction === 'down' ? 'decreased' : 'remained stable'} by ${trend.percentage}% in the last ${trend.period}.`
                        },
                        {
                            color: 'bg-red-500',
                            title: 'Regulatory Focus',
                            text: `${regulationsCount} regulatory documents account for ${regulationsPercent}% of all policies in the database.`
                        },
                        {
                            color: 'bg-green-500',
                            title: 'Sector Coverage',
                            text: `${sectorTrends[0]?.fullName || sectorTrends[0]?.sector || 'N/A'} is the most covered sector with ${sectorTrends[0]?.count || 0} policies.`
                        },
                        {
                            color: 'bg-blue-500',
                            title: 'Recent Activity',
                            text: `${policies.filter(p => new Date(p.published_date) >= new Date(new Date().setMonth(new Date().getMonth() - 6))).length} policies published in the last 6 months.`
                        },
                    ].map((insight) => (
                        <div key={insight.title} className="bg-white p-4 rounded-lg border border-blue-100 shadow-sm">
